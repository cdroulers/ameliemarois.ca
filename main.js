...
const startingWidth = innerWidth/1||document.documentElement.clientWidth/1||document.body.clientWidth/1;
const startingHeight = innerHeight/1||document.documentElement.clientHeight/1||document.body.clientHeight/1;
...
function responsiveReposition() {
  ...
  let currentWidth = innerWidth/1||document.documentElement.clientWidth/1||document.body.clientWidth/1;

  let currentHeight = innerHeight/1||document.documentElement.clientHeight/1||document.body.clientHeight/1;

  let xOffset = -(startingWidth-currentWidth)/2;
  let yOffset = -(startingHeight-currentHeight)/2;
  
  d3.select("body").select("g")
...
function main() {
  ...
  let width = innerWidth/1||document.documentElement.clientWidth/1||document.body.clientWidth/1;
  
  let height = innerHeight/1||document.documentElement.clientHeight/1||document.body.clientHeight/1;
  ...
  let force = d3.layout.force()
    .size([width, height])
    .charge(function(d) { return d._children ? -d.size / 100 : -innerWidth/5; })
    .linkDistance(function(d) { return d.target._children ? 80 : 25 || innerHeight; })
    .on("tick", tick);
